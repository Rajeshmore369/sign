...
DATASET_ROOT = os.environ.get("SIGN_BINARY_DATASET_ROOT", os.path.join(BASE_DIR, "datasets"))
TRAINING_ROOT = os.path.join(DATASET_ROOT, "sign2text_dataset_2.0")
TEST_ROOT = os.path.join(DATASET_ROOT, "test_data_2.0")
...
def letter_dir(root, variant, letter):
    return ensure_dir(os.path.join(root, variant, letter))


def image_count(root, variant, letter):
    target_dir = letter_dir(root, variant, letter)
    return len([name for name in os.listdir(target_dir) if name.lower().endswith(".jpg")])
...
#testing data
count = image_count(TEST_ROOT, "Gray_imgs", "A")
...
flag=False
suv=0
WHITE_TEMPLATE = np.full((400, 400, 3), 255, dtype=np.uint8)
...
while True:
    try:
        ...
        if interrupt & 0xFF == ord('n'):
            ...
            # test data
            count = image_count(TEST_ROOT, "Gray_imgs", p_dir)
        ...
        if flag==True:
            ...
            if step%2==0:
                ...
                # cv2.imwrite(os.path.join(letter_dir(TRAINING_ROOT, "Binary_imgs", p_dir), c_dir + str(count) + ".jpg"), img_final)
                # cv2.imwrite(os.path.join(letter_dir(TRAINING_ROOT, "Gray_imgs", p_dir), c_dir + str(count) + ".jpg"), img_final1)
                # cv2.imwrite(os.path.join(letter_dir(TRAINING_ROOT, "Gray_imgs_with_drawing", p_dir), c_dir + str(count) + ".jpg"), img_final2)

                # this is for testing data collection
                # cv2.imwrite(os.path.join(letter_dir(TEST_ROOT, "Binary_imgs", p_dir), c_dir + str(count) + ".jpg"), img_final)
                cv2.imwrite(os.path.join(letter_dir(TEST_ROOT, "Gray_imgs", p_dir), c_dir + str(count) + ".jpg"),
                            img_final1)
                cv2.imwrite(
                    os.path.join(letter_dir(TEST_ROOT, "Gray_imgs_with_drawing", p_dir), c_dir + str(count) + ".jpg"),
                    img_final2)
