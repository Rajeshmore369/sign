...
BASE_DIR = oss.path.dirname(oss.path.abspath(__file__))
DATASET_ROOT = oss.environ.get("SIGN_DATASET_ROOT", BASE_DIR)
OUTPUT_ROOT = oss.path.join(DATASET_ROOT, "AtoZ_3.1")


def ensure_letter_dir(letter):
    target_dir = oss.path.join(OUTPUT_ROOT, letter)
    oss.makedirs(target_dir, exist_ok=True)
    return target_dir


def image_count(letter):
    target_dir = ensure_letter_dir(letter)
    return len([name for name in oss.listdir(target_dir) if name.lower().endswith(".jpg")])
    ...
    try:
        ...
        if flag==True:
            ...
            if suv==180:
                flag=False
            if step%3==0:
                output_file = oss.path.join(ensure_letter_dir(c_dir), str(count) + ".jpg")
                cv2.imwrite(output_file, skeleton1)
