...
This repository runs most reliably with Python 3.10.
...
- Start the app from the repository root so it can find `cnn8grps_rad1_model.h5` and `white.jpg`.
- If `pyenchant` is unavailable, the app still starts, but spelling suggestions are disabled.
- Webcam access must be enabled in Windows privacy settings.

## Data Collection Script Paths

- `data_collection_final.py` saves images into the repository folder `AtoZ_3.1` by default.
- `data_collection_binary.py` saves into `datasets/test_data_2.0` by default.
- You can override those roots with environment variables:

  - `SIGN_DATASET_ROOT` for `data_collection_final.py`
  - `SIGN_BINARY_DATASET_ROOT` for `data_collection_binary.py`
